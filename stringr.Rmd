...
StringR is a package, which deals with string manipulation such as measuring or altering strings as well as searching in strings. StringR is thus mostly involved with data preprocessing. Furthermore, it can be used e.g. to determine the frequency of words, which can be useful in text mining.
...
This changes how you must use stringR, because you don't deal with a full text now, which can be searched, but thousands of different boxes.
...
Regular expressions are used in almost every programming language. They are a way to normalize the syntax of strings. More importantly, they can be used to express not a static word, but they could, more generally, define a group of words. For example, the set of words which end on "r" contains the words "letter", "bear", "beer" etc. So, regular expressions are often used not to match a single word, but also a set of words.
...
"both"), pad = " ") adds as a default white spaces either to the left or right side of a string. The width states how long the string should be after the adding of the white spaces. "Both" as a parameter means that added white space is split between the left and the right side of the string. With the pad argument any other character can be added.
...
For str_trim keep in mind, that if you want to delete all the whitespaces, there are other methods (s.str_sub or str_replace)
...
```
Then we can use the strsplit() function from base R to split the string into a list of strings (or see str_split).
...
We look at the common words love and recipe in the description. In which descriptions does which come first, or the pattern doesn't come up at all. 
...
As you can see, in the first column there is the entry belonging to the whole regular expression as a group. The string contains "love" as well as "beef". In the second column, the string matched from the first group is "love", in the third column "beef".

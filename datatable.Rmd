...
Let us now compare the runtime of some expression made in `data.table` and `tidyverse`.
As example we want to calculate the average height of the athletes per country.
...
olympics_height <- olympics[!is.na(height)]
timecomp <- microbenchmark(
country_tall <- olympics_height %>% 
                group_by(country) %>%
                summarise( avg_height = mean(height)) %>%
                arrange(-avg_height),


country_tall_dt <- olympics_height[, .(avg_height =  mean(height)) , by=country][order(-avg_height)]


)

timecomp %>% group_by(expr) %>% summarise(mean(time))

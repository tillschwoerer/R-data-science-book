...
fct_count(factor_clarity) %>%
  ...
  geom_col(aes(f, n))
```
If we want to order by the counts, we use the `fct_infreq` function.
```{r}
fct_count(factor_clarity) %>%
  ggplot(aes(clarity, n)) +          #the definition of aes. mapping in this line is only used to label the axes
  geom_col(aes(f, n))
...
The `fct_cross` function creates new factors levels of the combined values from the input factors. The keep_empty parameter allows us to create values without observation.
...
With the following function we can check wheter a specific level is included and get back a logical vector.

```{r}

fct_match(countries_in_1896, "Germany") %>% head(25)
```

### Shifting levels

If we want to shift the ordering of levels, we can ust the `fct_shift` function. This allows us shifting all levels by a particular number of steps in a direction
...
And here are some additional functions for reordering the levels in a fast way 

```{r}
f <- factor(c("b", "b", "a", "c", "c", "c"))
f
fct_inorder(f)    #by the order in which they first appear

f <- factor(1:3, levels = c("3", "2", "1"))
f
fct_inseq(f)      #by numeric value of level
```

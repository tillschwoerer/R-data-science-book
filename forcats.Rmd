...
With the function ***fct_c()*** factors with different levels can be combined to one factor covering all levels. First we create two factors which represent a number of Olympic athletes from different countries using the dataset olympic-games

```{r}
levels_country1 <- c("Finland", "Denmark", "Netherlands")
levels_country2 <- c("Finland", "Norway", "Sweden")
factor_countries_1 <- factor(olympic$country[1:2000], levels_country1)     # transforming a data slice into a factor
factor_countries_2 <- factor(olympic$country[2001:4000], levels_country2)  # transforming another data slice into a factor
```

Both factors differ both in their stored categorical data and in their set levels. A simple query makes this clear:

```{r}
factor_countries_1 %>%  fct_count()
factor_countries_2 %>%  fct_count()
```

Now we combine the factors with ***fct_c()***:

```{r}
fct_c(factor_countries_1, factor_countries_2) %>% fct_count()
```

Both the underlying categorical data and the set levels were combined in one factor.

### Standardize levels of different factors

```{r}
fct_unify(list(factor_countries_1, factor_countries_2)) %>% str()
```

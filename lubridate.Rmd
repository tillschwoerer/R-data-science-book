...
```{r correct NIEDERSCHLAGSINDIKATOR - action and check, echo=FALSE}
...
```{r correct RELATIVE_FEUCHTE - action and check, echo=FALSE}
...
```{r correct NIEDERSCHLAGSHOEHE - before, echo=FALSE}
values_of_ARN <- df %>% 
...
```
The negative values are set to 0 again.
```{r correct NIEDERSCHLAGSHOEHE - action and check, echo=FALSE}
...
```{r create a DURATION object, echo=FALSE}
...
```{r show start and end of summer interval, echo=FALSE}
...
```{r show start and end of summer interval - favourite, echo=FALSE}
sft <- "%d.%m.%Y-%H:%M"
...
```{r modify end before select data of autmn, echo=FALSE}

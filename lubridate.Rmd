...
df_sum_autmn_group <- df_group_autmn 
df_sum_autmn <- df_sum_autmn_group %>% 
...
df_sum_autmn_liter_group <- df_group_autmn 
df_sum_autmn_liter <- df_sum_autmn_liter_group %>%
            summarise(MENGE = sum(NIEDERSCHLAGSHOEHE[NIEDERSCHLAGSINDIKATOR==1])) %>% 
            filter(MENGE>0) %>%
            arrange( STATIONS_ID , JAHR, MONAT, TAG)
head(df_sum_autmn_liter, 10) 
```
